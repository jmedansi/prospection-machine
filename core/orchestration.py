...
def enabled_campagnes() -> list[dict]:
    ...
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT id, nom, envoi_auto FROM campagnes WHERE statut = 'actif' AND envoi_auto = 1 ORDER BY nom ASC"
        ).fetchall()
    return [dict(r) for r in rows]
...
def relances_due(campagne_id: int | None = None, now=None, *, objectif_id: int | None = None) -> list[dict]:
    ...
    with get_conn() as conn:
        rows = conn.execute(
            f"""SELECT p.id, p.statut,
                ...
                WHERE l.campagne_id = ? AND p.statut IN ({placeholders})
...
def run_relances(campagne_id: int | None = None,
                 ...
                 manual: bool = False, *,
                 objectif_id: int | None = None) -> dict:
