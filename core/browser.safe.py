...
Gestion captcha (valable pour tous les modules) :
    ...
    Comportement :
      ...
      - Attend 15 min que l'utilisateur résolve dans Chrome
      - Si résolu → retourne le même onglet, scraping reprend
      - Si timeout → ouvre un nouvel onglet propre, ferme l'onglet bloqué
"""
...
# Singleton pour le browser headless supprimé car sync_playwright n'est pas thread-safe !
# Utilisation de _sync_local.headless_browser à la place.
...
def cdp_tab(viewport: dict = None):
    ...
    Utilise browser.new_page() → profil par défaut (pas incognito).
    """
    try:
...
async def cdp_tab_headless_async(viewport: dict = None):
    """
    Ouvre un onglet dans une instance Chromium local 100% HEADLESS (Asynchrone).
    Singleton global pour économiser la RAM.
    """
    global _headless_browser_async, _headless_pw_async
    
    if _headless_browser_async is None:
        from playwright.async_api import async_playwright as _ap
        _headless_pw_async = await _ap().start()
        _headless_browser_async = await _headless_pw_async.chromium.launch(
            headless=True,
            args=[
                '--no-sandbox', 
                '--disable-dev-shm-usage', 
    ...
    ctx = None
    try:
        ctx = await _headless_browser_async.new_context(locale="fr-FR", viewport=viewport)
        page = await ctx.new_page()
        yield page
    finally:
        if ctx:
...
# Cache par event loop — évite l'erreur "NoneType object has no attribute send"
# quand plusieurs loops (ex: threads différents) utilisent le même browser.
_async_browsers = {}


async def get_async_browser(force_restart: bool = False):
    """Retourne l'instance Browser connectée par CDP (Async). Singleton par loop."""
    loop = asyncio.get_running_loop()
    
    # Validation du cache existant
    if not force_restart and loop in _async_browsers:
        browser, pw = _async_browsers[loop]
        try:
            # Test léger de connexion
            _ = browser.contexts
            return browser
        except:
            logger.warning("[browser] Browser async en cache défectueux, réouverture...")
            del _async_browsers[loop]
    ...
    try:
        ...
        _async_browsers[loop] = (browser, pw)
        logger.info(f"[browser] CDP async connecté (port {_CDP_PORT}, loop={id(loop)})")
        return browser
    except Exception as e:
        await pw.stop()
...
async def close_async_browsers():
    """Ferme proprement le Playwright async de la boucle courante pour eviter les fuites de connexion CDP."""
    loop = asyncio.get_running_loop()
    if loop in _async_browsers:
        browser, pw = _async_browsers[loop]
        try:
            await browser.close()
        except Exception:
        ...
        try:
            ...
            pass
        del _async_browsers[loop]
        logger.info(f"[browser] Instance async fermée et nettoyée pour la loop {id(loop)}")
...
def handle_captcha_sync(page, label: str = "") -> object:
    ...
    Même comportement : attend 15 min, ouvre un nouvel onglet si timeout.
    ...
    logger.warning(f"{tag}CAPTCHA non résolu après 15 min — nouvel onglet ouvert")
    _tg_notify(f"⏱️ CAPTCHA non résolu après 15 min ({label}) — ouverture d'un nouvel onglet, on continue.")

    try:
        new_page = page.context.new_page()
        return new_page
    except Exception as e:
        logger.error(f"{tag}Impossible d'ouvrir un nouvel onglet : {e}")
        return page
