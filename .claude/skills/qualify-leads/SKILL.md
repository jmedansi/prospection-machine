...
> **en place** les 4 colonnes IA de chaque ligne : `Opportunite`, `Score`, `Signaux`, `Reco`.
...
| `Reco` | texte | **`a_contacter`** / **`a_verifier`** / **`a_ecarter`** — recommandation de traitement (voir §2.5 ci-dessous) |
...
### 2.5 Recommandation `Reco` (à contacter / à vérifier / à écarter)

| Valeur | Quand | Critère |
|---|---|---|
| `a_contacter` | Opportunité nette | Opportunite ≥ 3 (haut priorité : site absent/cassé/daté) ET contact exploitable (email ou téléphone valide) |
| `a_verifier` | Doute, bande médiane | Opportunite = 2 (score 16-39) OU contact incomplet OU données incertaines — jugement humain requis |
| `a_ecarter` | Raison structurelle forte | Domaine à vendre / parking (pas un vrai commerce) ; établissement fermé/disparu ; hors zone/cible ; doublon confirmé ; aucune donnée exploitable (ni email ni téléphone ni site) |

> ⚠️ **Ne jamais écarter sur la seule base de l'état du site** (règle d'or §1) : un site daté
> est un **angle d'approche**, pas un motif d'exclusion. L'exclusion finale est manuelle
> (dashboard) ; cette colonne est une recommandation pour faciliter le tri.
> Laisser vide si non analysé (aucune donnée → pas de recommandation).
...
2. Remplis `Opportunite`, `Score`, `Signaux`, `Reco` sur la même ligne, même `Id`.
...
qui relit le CSV et réintègre `Score` dans `prospects.score` (v2) + `Opportunite/Signaux/Reco` dans
`prospects.data_extra` (clés `ia_opportunite`, `ia_signaux`, `ia_reco`). L'`Id` du CSV est l'id du prospect **v2**. Aucune action réseau
...
6. `Reco` : ne pas écarter sur la base de l'état seul (règle d'or §2.5). Laisser vide si données insuffisantes.
