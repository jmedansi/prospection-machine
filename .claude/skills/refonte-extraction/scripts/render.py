...
with sync_playwright() as p:
    ...
    try:
        ...
        def safe_eval(fn, default=None):
            """Évalue sans planter si une navigation détruit le contexte ; retente une fois."""
            try:
                ...
                try:
                    ...
                    return default
        try:
            safe_eval("async()=>{await new Promise(r=>{let y=0;const t=setInterval(()=>{window.scrollBy(0,700);y+=700;if(y>=document.body.scrollHeight){clearInterval(t);r();}},120);});}")
        except Exception:
            pass
        ...
        try:
            ...
            pass
        try: page.screenshot(path=str(outdir/"screenshots"/"above-fold.png"))
        except Exception: pass
        try: page.screenshot(path=str(outdir/"screenshots"/"full-page.png"), full_page=True)
        except Exception: pass
        (outdir/"raw"/"computed-styles.json").write_text(json.dumps(safe_eval(JS_STYLES, {}), indent=2), encoding="utf-8")
        images=safe_eval(JS_IMAGES, []) or []
