...
def match_events_message_ids(message_ids: list) -> list:
    ...
    with get_conn() as conn:
        rows = conn.execute(
            f"""SELECT prospect_id, campagne_id FROM prospect_events
...
def find_parent_event(parsed: dict) -> dict | None:
    """Event sortant auquel une réponse entrante se rattache.

    Priorité : In-Reply-To exact, sinon dernier Message-ID des References
    (chaines des ancêtres du plus ancien au plus récent → dernier = parent).
    Retourne le row prospect_events complet (id, thread_id, message_id...).
    """
    refs = re.split(r'\s+', ' '.join([parsed.get('in_reply_to') or '',
    ...
    with get_conn() as conn:
        ...
        if in_reply_to:
            row = conn.execute(
                "SELECT * FROM prospect_events WHERE message_id = ? ORDER BY id DESC LIMIT 1",
                (in_reply_to,),
            ).fetchone()
            if row:
                return dict(row)
        placeholders = ','.join('?' for _ in ids)
        row = conn.execute(
            f"SELECT * FROM prospect_events WHERE message_id IN ({placeholders}) ORDER BY id DESC LIMIT 1",
            ids,
        ).fetchone()
...
def match_prospect(parsed: dict) -> dict | None:
    ...
    with get_conn() as conn:
        row = conn.execute(
            f"""SELECT p.id, p.statut, l.campagne_id FROM prospects p
                JOIN listes l ON p.liste_id = l.id
                WHERE LOWER(p.email) = ? AND p.statut IN {SENT_CLAUSE}
...
def handle_reply(parsed: dict, prospect: dict, kind: str = 'reponse',
    ...
    # Rattachement au fil de conversation (threading RFC 2822)
    parent = find_parent_event(parsed)
    parent_event_id = parent['id'] if parent else None
    thread_id = (parent['thread_id'] or parent['id']) if parent else None
    with get_conn() as conn:
        cur = conn.execute(
            """INSERT INTO prospect_events
               (prospect_id, campagne_id, event_type, payload, message_id,
                in_reply_to, references_header, direction, parent_event_id, thread_id)
               VALUES (?,?,?,?,?,?,?,'in',?,?)""",
            (pid, cid, kind, __import__('json').dumps(payload, ensure_ascii=False),
             parsed.get('message_id') or None,
             parsed.get('in_reply_to') or None,
             parsed.get('references') or None,
             parent_event_id, thread_id),
        )
        conn.commit()
        event_id = cur.lastrowid
    if kind == 'reponse':
        _notify_reponse(prospect, parsed, snippet)
    logger.info("[replies] %s prospect #%s campagne #%s (%s → %s)",
                kind, pid, cid, parsed.get('from_addr') or '?',
