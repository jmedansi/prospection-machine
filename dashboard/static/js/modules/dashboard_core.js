        ...
        async function toggleContactPill(leadId, method, el) {
            ...
            try {
                ...
                if (typeof triggerGlobalLeadRefresh === 'function') {
                    triggerGlobalLeadRefresh(leadId);
                }
            } catch (e) {
        ...
        function nav(page, el) {
            ...
            if(target) {
                ...
                // Restauration du sous-onglet depuis le localStorage si non spécifié par alias
                const savedSubtab = localStorage.getItem('pm_subtab_section-' + page);
                const subtabToUse = subtab || savedSubtab;

                if (subtabToUse) {
                    const tabBtn = target.querySelector(`.section-tab[onclick*="'${subtabToUse}'"]`);
                    if (tabBtn) {
                    ...
                    // Si aucun sous-onglet n'est sauvegardé, chercher celui qui a la classe active par défaut
                    const activeTabBtn = target.querySelector('.section-tab.active');
                    if (activeTabBtn) {
                        const match = activeTabBtn.getAttribute('onclick').match(/'([^']+)'/);
                        if (match) {
                            switchSectionTab(match[1], activeTabBtn);
                        } else {
                            _loadDataForSection(page);
                        }
                    } else {
        ...
        function switchSectionTab(tabName, el) {
            ...
            // Sauvegarder le sous-onglet actif pour cette section
        ...
        async function loadPanelContent(leadId, tab) {
            ...
            if (leadFromCache) {
                ...
                if (tab === 'audit') {
                    ...
                    if ((leadFromCache.objectif || 'general') === 'web' && document.getElementById('ia-maq-' + leadIdNum)) {
            ...
            try {
                ...
                if (tab === 'audit') {
                    ...
                    if ((lead.objectif || 'general') === 'web' && document.getElementById('ia-maq-' + leadIdNum)) {
                        _loadLeadMaquette(leadIdNum, 'ia-maq-' + leadIdNum);
                    }
        ...
        /** Prospection IA : assistant (qualification + email rédigé par l'IA). Jamais d'envoi ici. */
        ...
        /** Bouton d'action pipeline par lead (panneau latéral). Appelle window.IaActions. */
        ...
        /**
         * Bloc « Prospection IA » enrichi pour la pipeline.
         * Affiche : badge objectif + badge cas (avec/sans site), Opportunité/Score/Signaux,
         * la maquette réelle (capture.png) chargée via /api/ia/lead/<id>/maquette,
         * l'aperçu du mail IA, et les boutons d'action Qualifier / Maquette / Rédiger.
         */
        function _renderIAPanel(lead) {
            ...
            const obj = (lead.objectif || 'general');
            ...
            const maquetteHtml = obj === 'web' ? `
                <div style="margin-top:14px;border-top:1px dashed #cbd5e1;padding-top:12px">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
                        ...
                        <a href="javascript:void(0)" onclick="window.IaActions && IaActions.leadMaquette(${id}).then(d=>{if(d&&d.files&&d.files.length)dialogOpenMaquette(${id});})" style="font-size:11px;color:#6366f1;font-weight:700">Ouvrir la maquette →</a>
                    </div>
        ...
        /** Injecte la maquette réelle (capture) dans le panneau une fois chargée. */
...
<div class="panel-section" style="margin-bottom:32px">
        ...
        function renderSuiviPanel(lead) {
            ...
            <div class="panel-section" style="margin-bottom:14px">
                ...
                ${typeof _ulPanelTraitement === 'function' ? _ulPanelTraitement(lead) : `<div style="font-size:12px;font-weight:600">${escHtml(lead.statut || '—')}</div>`}
            </div>
        ...
        window.sendLeadDirectReply = async function sendLeadDirectReply(leadId) {
            ...
            if (!body) {
                if (typeof showToast === 'function') showToast('Veuillez saisir un corps de message', 'warning');
            ...
            try {
                ...
                const data = await resp.json();
                if (data.success) {
                    if (typeof showToast === 'function') showToast('Email envoyé avec succès !', 'success');
                    if (bodyEl) bodyEl.value = '';
                    loadPanelContent(leadId, 'suivi');
                } else {
                    if (typeof showToast === 'function') showToast('Erreur : ' + (data.error || 'Échec envoi'), 'error');
        ...
        async function previewEmail(leadId) {
            ...
            if (win) win.document.write(lead.email_corps);
        }

        /** Ouvre la maquette IA (index.html) d'un lead dans une modale plein écran. */
        function dialogOpenMaquette(leadId) {
            if (!window.IaActions) return;
            window.IaActions.leadMaquette(leadId).then(d => {
                if (!d || !d.liste || !d.files.length) {
                    if (window.showToast) showToast('Aucune maquette générée pour ce lead.', 'error');
                    return;
                }
...
function pollAuditCompletion(leadId, attempts) {
    ...
    fetch('/api/audit/status')
        ...
        // Fonction globale de rafraîchissement des vues de données après modification d'un lead
        function triggerGlobalLeadRefresh(leadId) {
            console.log('[Global Refresh] Refreshing tables after update of lead', leadId);
            
            // 1. Rafraîchir la table de la section Campagne active (Cockpit / CRM)
            if (typeof loadCampaignTable === 'function') {
                loadCampaignTable();
            }
            
            // 2. Rafraîchir les leads unifiés (Leads Station) si la table ou les variables existent
            if (typeof unifiedLeadsLoad === 'function') {
                if (typeof _ul !== 'undefined' && _ul.page) {
            ...
            // 3. Rafraîchir la liste de leads active (vue Listes) si ListsModule est présent
            if (window.ListsModule && typeof window.ListsModule.loadListLeads === 'function') {
                window.ListsModule.loadListLeads();
            }
            
            // 4. Rafraîchir le suivi CRM ou les tableaux de suivi
            if (typeof loadCRM === 'function') {
                loadCRM();
            }
            if (typeof loadTrackingBoard === 'function') {
                loadTrackingBoard(true); // silent
            }
        }
        window.triggerGlobalLeadRefresh = triggerGlobalLeadRefresh;

        // ─── CRM (vue Suivi) ──────────────────────────────────────────────────
        let _crmFilter = 'tous';
        const CRM_FILTER_TOQPID = {
            tous: '', ouverts: 'opened', cliques: 'clicked',
            repondus: 'replied', positifs: 'replied', bounces: 'bounce', spam: 'spam'
        };
        const CRM_FILTER_LABEL = {
            '' : 'tous', opened: 'ouverts', clicked: 'cliques',
            replied: 'repondus', bounce: 'bounces', spam: 'spam'
        };

        async function loadCRMCounts() {
            try {
                if (typeof window.loadDashboardStats === 'function') await window.loadDashboardStats();
                else if (typeof loadDashboardStats === 'function') await loadDashboardStats();
            } catch (e) { console.error('[loadCRMCounts]', e); }
        }

        function _crmRow(email) {
            if (!email) return '';
            const badges = [];
            if (email.ouvert) badges.push('<span style="font-size:9px;padding:1px 5px;border-radius:3px;background:#3b82f620;color:#3b82f6">Ouvert</span>');
            if (email.clique) badges.push('<span style="font-size:9px;padding:1px 5px;border-radius:3px;background:#10b98120;color:#10b981">Cliqué</span>');
            if (email.repondu) badges.push('<span style="font-size:9px;padding:1px 5px;border-radius:3px;background:#ef444420;color:#ef4444">Répondu</span>');
            if (email.bounce) badges.push('<span style="font-size:9px;padding:1px 5px;border-radius:3px;background:#6b728020;color:#6b7280">Bounce</span>');
            if (email.spam) badges.push('<span style="font-size:9px;padding:1px 5px;border-radius:3px;background:#f59e0b20;color:#f59e0b">Spam</span>');
            if (!badges.length) badges.push('<span style="font-size:9px;padding:1px 5px;border-radius:3px;background:var(--ink3)18;color:var(--ink3)">Envoyé</span>');
            const date = email.date_envoi ? email.date_envoi.replace('T', ' ').slice(0, 16) : '';
            const nom = email.nom || '—';
            return `<tr style="cursor:pointer">
                <td><strong>${nom}</strong><div style="font-size:10px;color:var(--ink3)">${email.ville || email.secteur || ''}</div></td>
                <td style="font-size:11px;white-space:nowrap">${date}</td>
                <td>${badges.join(' ')}</td>
                <td style="white-space:nowrap">${(email.repondu ? '🗿 Réponse reçue' : '—')}</td>
                <td>${email.rdv_confirme ? '✅ RDV' : ''}</td>
            </tr>`;
        }
        ...
        async function loadCRM() {
            ...
            if (tbody) {
                tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:1rem">Chargement...</td></tr>';
            }
            const listId = document.getElementById('crm-filter-list')?.value || '';
            try {
                const statut = CRM_FILTER_TOQPID[_crmFilter] || '';
                const params = new URLSearchParams({ statut, limit: '50' });
                ...
                tbody.innerHTML = emails.length
                    ? emails.map(_crmRow).join('')
                    : '<tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--ink3)">Aucun email trouvé avec ce filtre</td></tr>';
            } catch (e) {
                console.error('[loadCRM]', e);
                if (tbody) tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--error)">Erreur de chargement</td></tr>';
            }
        }

        function setCRMFilter(filter, btn) {
            _crmFilter = filter || 'tous';
            const wrap = document.getElementById('crm-filters');
            if (wrap) wrap.querySelectorAll('.btn.active').forEach(b => b.classList.remove('active'));
            if (btn) btn.classList.add('active');
            loadCRM();
        }

        window.loadCRM = loadCRM;
        window.loadCRMCounts = loadCRMCounts;
