...
async function loadCampaigns() {
    try {
        ...
        const activeSector = typeof _activeSector !== 'undefined' ? _activeSector : '';
        let url = '/api/campaigns';
        const params = new URLSearchParams();
        if (dateFilter) { params.set('date_start', dateFilter); params.set('date_end', dateFilter); }
...
async function loadSectors() {
    try {
        ...
        const current = _activeSector || '';
        let html = '<option value="">Tous secteurs</option>';
        (d.sectors || []).forEach(s => {
            const val = s.value || s;
            const lbl = s.label || s;
            html += `<option value="${escHtml(val)}" ${val === current ? 'selected' : ''}>${escHtml(lbl)}</option>`;
        });
        select.innerHTML = html;
...
async function loadCampaignsForSector(sector) {
    _activeSector     = sector || null;
    _activeCampaignId = null;

    const campSelect = document.getElementById('global-campaign-select');
    if (campSelect) campSelect.value = '';

    // Synchro avec le filtre secteur de la table Leads
    const leadsSector = document.getElementById('ul-filter-sector');
    if (leadsSector) leadsSector.value = sector || '';

    // Recharger le dropdown campagnes sans filtre secteur
    loadCampaigns();
    refreshAll();
    if (typeof unifiedLeadsLoad === 'function') unifiedLeadsLoad(1);
}
