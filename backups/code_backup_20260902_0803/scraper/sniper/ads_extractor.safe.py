...
async def _extract_from_google(
    page, keyword: str, country: str, max_per_kw: int, pages_per_kw: int = _MAX_PAGES_SAFETY,
    ...
    for page_idx in range(max_pages):
        ...
        try:
            ...
            if random.random() < 0.6:
                await page.evaluate("() => window.scrollTo(0, random.randint(0, 200))")
        ...
        for kw in keywords:
            ...
            try:
                ...
                domains, page, was_blocked = await _extract_from_google(
                    page, kw, country, max_per_kw, pages_per_kw,
                ...
                if not domains:
                    if was_blocked:
                        if use_bing_fallback:
                            ...
                            domains = await _extract_from_bing(page, kw, country, max_per_kw)
                        else:
                ...
                # Fermer l'onglet, mais pas le contexte (le profil reste intact)
                try:
                    await page.close()
                except Exception:
                    pass
