...
def transition_prospect(prospect_id: int, nouveau_statut: str, reason: str = '', payload: dict = None) -> dict:
    ...
    with get_conn() as conn:
        row = conn.execute(
            "SELECT id, objectif_id, statut, statut_meta FROM prospects WHERE id = ?", (prospect_id,)
        ).fetchone()
        if not row:
        ...
        conn.execute(
            "INSERT INTO prospect_events (prospect_id, objectif_id, event_type, payload) VALUES (?, ?, 'status_change', ?)",
            (prospect_id, row['objectif_id'], json.dumps({
                'from': ancien,
                'to': nouveau_statut,
                'reason': reason,
                **payload,
