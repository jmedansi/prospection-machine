...
async def main_async(argv=None):
    ...
    parser.add_argument("--objectif", type=str, default="",
                        help="Objectif v2 (nom ou id) dans lequel ranger les leads collectés "
                             "(ex: « Refonte site web »). Créé s'il n'existe pas.")
    if argv is not None:
    ...
    # ── Objectif v2 cible (résolu UNE fois avant le premier lead) ─────────
    v2_objectif = None
    v2_counters = {'importes': 0, 'doublons': 0, 'supprimes': 0}
    if args.objectif:
        try:
            from core.objectif_registry import resolve_or_create_objectif
            v2_objectif = resolve_or_create_objectif(args.objectif)
            print(f"   [V2] Objectif cible : #{v2_objectif[0]} « {v2_objectif[1]} »" if v2_objectif
                  else f"   [V2] ⚠️ Impossible de rattacher l'objectif « {args.objectif} »")
        except Exception as e:
            print(f"   [V2] ⚠️ Échec résolution objectif « {args.objectif} » : {e}")
            v2_objectif = None
    ...
    while zones_queue and not _objectif_atteint() and passe_num < args.max_passes:
        ...
        try:
            ...
            for place in nouveaux:
                ...
                # Miroir v2 : insertion du lead enrichi DANS l'objectif choisi
                if v2_objectif:
                    try:
    ...
    if v2_objectif:
        print(f"   [V2] Objectif     : #{v2_objectif[0]} « {v2_objectif[1]} »")
