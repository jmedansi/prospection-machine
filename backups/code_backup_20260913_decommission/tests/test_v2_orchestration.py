...
def _objectif(nom='Refonte site web', envoi_auto=1, validation_telegram=0):
    ...
    oid, _nom = resolve_or_create_objectif(nom)
    from database.connection import get_conn
    with get_conn() as c:
        c.execute("UPDATE objectifs SET envoi_auto=?, validation_telegram=? WHERE id=?", (envoi_auto, validation_telegram, oid))
        c.commit()
    return oid


def _prospect(oid, email='contact@dupont.fr', oppose=False, ecarte=False):
...
def test_kill_switch_global_arrete_tout(tmp_db, fake_gateway):
    oid = _objectif()
    _prospect(oid)
    from database import objectifs as obj_repo
    obj_repo.set_auto_send_enabled(False)
...
def test_scheduler_ignore_objectif_envoi_auto_0(tmp_db, fake_gateway, monkeypatch):
    ...
    _prospect(oid_off)
    from database import objectifs as obj_repo
    obj_repo.set_auto_send_enabled(True)
    from core.orchestration import run_auto_send
    res = run_auto_send()
...
def test_kill_switch_se_relit_depuis_la_base(tmp_db):
    from database import objectifs as obj_repo
    assert obj_repo.get_auto_send_enabled() is True
    obj_repo.set_auto_send_enabled(False)
    assert obj_repo.get_auto_send_enabled() is False
    obj_repo.set_auto_send_enabled(True)
    assert obj_repo.get_auto_send_enabled() is True