...
def _prospect(oid, email='contact@dupont.fr', statut='qualifie'):
    from database import prospects_repo
    pid = prospects_repo.insert_prospect(oid, nom='Boulangerie Dupont', email=email,
...
def test_get_templates_filtre_actif_et_objectif(tmp_db):
    ...
    _del_seed_templates()
    oid = _objectif()
    tr.add_or_update(objectif_id=oid, nom='Initial obj', objet='O {{prenom}}', corps='C',
    ...
    tid = tr.get_templates(objectif_id=oid)[-1]['id']
    tr.set_actif(tid, False)

    temps = tr.get_templates(objectif_id=oid)
    ...
    noms2 = [t['nom'] for t in tr.get_templates(objectif_id=oid2)]
...
def test_get_step_priorite_objectif_puis_generique(tmp_db):
    from envoi import template_registry as tr
    oid = _objectif()
    tr.add_or_update(objectif_id=oid, nom='Dédié', objet='Dédié {{prenom}}', corps='c', position=0)
    ...
    assert step2['objectif_id'] is None
