...
def _objectif(nom='Refonte site web', validation_telegram=0, max_touches=3):
    ...
    oid, _ = resolve_or_create_objectif(nom)
    from database.connection import get_conn
    with get_conn() as c:
        c.execute("UPDATE objectifs SET validation_telegram=?, max_touches=? WHERE id=?", (validation_telegram, max_touches, oid))
        c.commit()
    return oid
...
def _prospect(oid, email='contact@dupont.fr'):
    ...
    return prospects_repo.insert_prospect(oid, nom='Boulangerie Dupont', email=email,
...
def _seed_template_step(oid, position, delai_jours=0, corps='Corps relance {{prenom}}'):
    ...
    return template_registry.add_or_update(objectif_id=oid, position=position,
...
def _to_en_sequence(pid, oid, days_ago=0):
    ...
    with get_conn() as c:
        ...
        c.execute("INSERT INTO prospect_events (prospect_id, objectif_id, event_type, payload, created_at) VALUES (?,?,?,?,?)",
                  (pid, oid, 'initial', '{}', ts))
        c.commit()
