        ...
        with get_conn() as conn:
            ...
            rows = conn.execute(f"""
                SELECT
                    ...
                    ll.created_at, ll.updated_at, ll.objectif,
...
def api_lists_create():
    ...
    try:
        ...
        objectif = (data.get("objectif") or "general").strip()
        if objectif not in ("web", "general", "tous"):
            objectif = "general"

        with get_conn() as conn:
            cur = conn.execute(
                "INSERT INTO lead_lists (nom, description, couleur, icone, objectif) VALUES (?, ?, ?, ?, ?)",
                (nom, description, couleur, icone, objectif)
            )
            conn.commit()
            list_id = cur.lastrowid
