...
function _globalFilters(extra = {}) {
    ...
    // Objectif v2 actif (sélecteur top) → stats cockpit (les endpoints legacy ignorent)
    if (window._ul && window._ul.v2ObjectifId) p.set('objectif_id', window._ul.v2ObjectifId);
...
async function loadStats() {
    try {
        ...
        // ROI (Phase 4.4)
        if (typeof d.projected_roi !== 'undefined') {
            const roiFormatted = new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(d.projected_roi);
            setText('stat-roi-projected', roiFormatted);
        }
...
async function refreshAll() {
    ...
    await Promise.all([
        ...
        typeof unifiedLeadsLoad === 'function' ? unifiedLeadsLoad(1) : Promise.resolve(),
