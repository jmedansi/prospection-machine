...
window.ListsModule = (function () {
            ...
            <div class="list-sidebar-item ${lst.id === _state.activeListId ? 'active' : ''}"
                ...
                <div class="list-sidebar-info">
                    ...
                    <div class="list-sidebar-meta">
                        <span style="opacity:.85">#${lst.id}</span> · ${lst.nb_leads || 0} lead${lst.nb_leads !== 1 ? 's' : ''}
                    </div>
                </div>
    ...
    function selectList(listId) {
        ...
        const idBadge = document.getElementById('list-active-id');
        if (idBadge) idBadge.textContent = lst ? `#${lst.id}` : '#—';
        const objBadge = document.getElementById('list-active-objectif');
        if (objBadge) {
            objBadge.textContent = (lst?.objectif === 'web' ? 'Web · maquette+email' : 'Général');
            objBadge.style.display = 'inline-block';
        }
    ...
    function _renderLeadRow(l) {
        ...
        return `<tr onclick="openLeadPanel(${l.id})" style="cursor:pointer">
            ...
            <td class="col-id" style="font-family:monospace;font-size:11px;color:var(--ink3)" title="ID lead : ${l.id}">
                ${l.id}
            </td>
    ...
    function openCreateModal() {
        ...
        const obj = document.getElementById('list-create-objectif');
        if (obj) obj.value = 'general';
        _initPickerGrids();
    ...
    function openEditModal(listId) {
        ...
        const obj = document.getElementById('list-create-objectif');
        if (obj) obj.value = lst.objectif || 'general';
        _initPickerGrids();
    ...
    async function saveList() {
        ...
        const payload = {
            ...
            objectif: document.getElementById('list-create-objectif')?.value || 'general',
        };
