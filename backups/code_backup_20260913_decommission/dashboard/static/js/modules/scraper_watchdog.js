    ...
    if (!anyRunning) {
        ...
        Object.entries(bars).forEach(([key, el]) => {
            ...
            if (s && s.running) {
                ...
                const currentCount = (s.processed !== undefined && s.processed !== null) ? s.processed : (s.accepted || 0);
                const totalCount = s.total || 0;
                const pct = totalCount > 0 ? Math.min(100, Math.round((currentCount / totalCount) * 100)) : null;
                const pctStr = pct !== null ? `${pct}%` : '…';
                ...
                if (barEl) barEl.style.width = (pct !== null ? pct : 30) + '%';
                if (textEl) {
                    if (key === 'audit' || key === 'enrichment' || key === 'sending') {
                        textEl.innerHTML = `<strong>${currentCount}</strong>/${totalCount || '?'}` + (s.failed ? ` · <span style="color:var(--red)">${s.failed} ❌</span>` : '');
                    } else {
                        textEl.innerHTML = `<strong>${currentCount}</strong>/${totalCount || '?'} leads` + (s.phase_detail ? ` · <span style="font-size:11px;opacity:0.8">${s.phase_detail}</span>` : '');
                    }
                }
            } else {
