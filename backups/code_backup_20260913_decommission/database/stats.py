...
def _stats_v2(conn, objectif_id: int | None, date_start: str | None = None, date_end: str | None = None) -> dict:
    """Métriques cockpit du modèle v2 (machine à états prospects).

    `objectif_id=None` → vue GLOBALE (tous les objectifs). `objectif_id` → un seul.
    Même contrat de clés que la branche legacy (lecteurs : routes/stats.py, stats.js,
    api.js, dashboard_core.js, sidebar).
    """
    where = "WHERE objectif_id = ?"
    params = [objectif_id]
    ...
    total = conn.execute(f"SELECT COUNT(*) AS c FROM prospects {where}", params).fetchone()['c'] or 0
    avec_site = conn.execute(
        f"SELECT COUNT(*) AS c FROM prospects {where} AND site_web IS NOT NULL AND site_web != ''",
        params).fetchone()['c'] or 0
    avec_email = conn.execute(
        f"SELECT COUNT(*) AS c FROM prospects {where} AND email IS NOT NULL AND email != ''",
        params).fetchone()['c'] or 0
    qualifie = conn.execute(
        f"SELECT COUNT(*) AS c FROM prospects {where} AND statut = 'qualifie' AND ecarte = 0",
        params).fetchone()['c'] or 0
    envoye = conn.execute(
        f"SELECT COUNT(*) AS c FROM prospects {where} AND ecarte = 0 AND statut IN {SENT}",
        params).fetchone()['c'] or 0
    repondu = conn.execute(
        f"SELECT COUNT(*) AS c FROM prospects {where} AND ecarte = 0 AND statut IN {REPLIED}",
        params).fetchone()['c'] or 0
    rdv = conn.execute(
        f"SELECT COUNT(*) AS c FROM prospects {where} AND ecarte = 0 AND statut = 'rdv_obtenu'",
        params).fetchone()['c'] or 0
