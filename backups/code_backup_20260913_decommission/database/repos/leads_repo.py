...
class LeadsRepo:
    ...
    def get_all(self, statut: str = "tous", limit: int = 500,
                 ...
                 objectif: str = "tous", show_ecartes: bool = False,
                 show_desinscrits: bool = False) -> dict:
        """
        ...
        try:
            with get_conn() as conn:
                base = """
                    ...
                    LEFT JOIN leads_audites la ON la.lead_id = lb.id
                """
                where, params = self._build_filters(statut, site, email, sector, search, campaign_id, campaign_ids, date_start, date_end, source=source, tag=tag, score=score, notes=notes, list_id=list_id, objectif=objectif, show_ecartes=show_ecartes, show_desinscrits=show_desinscrits)
    ...
    def set_objectif(self, lead_id: int, objectif: str) -> bool:
        """Définit l'objectif d'un lead ('web' | 'general')."""
        if objectif not in ("web", "general"):
            raise ValueError("objectif doit être 'web' ou 'general'")
        try:
            with get_conn() as conn:
                cur = conn.execute(
                    "UPDATE leads_bruts SET objectif=? WHERE id=?", (objectif, lead_id)
                )
                conn.commit()
                return cur.rowcount > 0
        except Exception as e:
            logger.error(f"LeadsRepo.set_objectif({lead_id}) → {e}")
            return False

    def set_categorie(self, lead_id: int, categorie: str | None) -> bool:
        """Affecte une catégorie (étudiée) à un lead, ou la retire si None."""
        try:
            with get_conn() as conn:
                cur = conn.execute(
                    "UPDATE leads_bruts SET categorie=? WHERE id=?", (categorie, lead_id)
                )
                conn.commit()
                return cur.rowcount > 0
        except Exception as e:
            logger.error(f"LeadsRepo.set_categorie({lead_id}) → {e}")
            return False

    def set_ecarte(self, lead_id: int, ecarte: bool) -> bool:
        """Écarte / réintègre un lead (hors flux d'envoi par défaut)."""
        try:
            with get_conn() as conn:
                cur = conn.execute(
                    "UPDATE leads_bruts SET ecarte=? WHERE id=?", (1 if ecarte else 0, lead_id)
                )
                conn.commit()
                return cur.rowcount > 0
        except Exception as e:
            logger.error(f"LeadsRepo.set_ecarte({lead_id}) → {e}")
            return False

    def set_desinscrit(self, lead_id: int, desinscrit: bool) -> bool:
        """Marque un lead comme désinscrit (opposition) / le remet en circulation."""
        try:
            with get_conn() as conn:
                cur = conn.execute(
                    "UPDATE leads_bruts SET desinscrit=? WHERE id=?",
                    (1 if desinscrit else 0, lead_id)
                )
                conn.commit()
                return cur.rowcount > 0
        except Exception as e:
            logger.error(f"LeadsRepo.set_desinscrit({lead_id}) → {e}")
            return False

    def set_ia_qualification(self, lead_id: int, opportunite=None, score=None, signaux=None) -> int | None:
        """Écrit le retour I.A. (Opportunite/Score/Signaux). Retourne le nombre de colonnes mises à jour."""
        if opportunite is None and score is None and signaux is None:
            return 0
        sets, params = [], []
        if opportunite is not None:
            sets.append("ia_opportunite=?"); params.append(int(opportunite))
        if score is not None:
            sets.append("ia_score=?"); params.append(int(score))
        if signaux is not None:
            sets.append("ia_signaux=?"); params.append(str(signaux))
        params.append(lead_id)
        try:
            with get_conn() as conn:
                cur = conn.execute(f"UPDATE leads_bruts SET {', '.join(sets)} WHERE id=?", params)
                conn.commit()
                return cur.rowcount
        except Exception as e:
            logger.error(f"LeadsRepo.set_ia_qualification({lead_id}) → {e}")
            return None
    ...
    def _build_filters(self, statut: str, site: str, email: str, sector: str, search: str = "",
                       ...
                       notes: str = "tous", list_id: int | None = None,
                       objectif: str = "tous", show_ecartes: bool = False,
                       show_desinscrits: bool = False):
        clauses, params = [], []
        
        # Objectif (web | general)
        if objectif in ("web", "general"):
            clauses.append("lb.objectif = ?")
            params.append(objectif)

        # Par défaut, masquer les leads écartés / désinscrits (hors flux d'envoi)
        if not show_ecartes:
            clauses.append("COALESCE(lb.ecarte, 0) = 0")
        if not show_desinscrits:
            clauses.append("COALESCE(lb.desinscrit, 0) = 0")
    ...
    def _normalize(self, d: dict) -> dict:
        ...
        return {
            ...
            "ecarte":            bool(d.get("ecarte")),
            "desinscrit":        bool(d.get("desinscrit")),
            # Qualification I.A. (passerelle IA ⇄ Dashboard)
            "ia_opportunite":    d.get("ia_opportunite"),
            "ia_score":          d.get("ia_score"),
            "ia_signaux":        d.get("ia_signaux"),
    ...
    def _get_kanban_status(self, d: dict) -> str:
        ...
        # 4. Intégrité de l'audit ou qualification IA
        ...
        # Qualification IA (fallback quand pas d'audit formel)
        ia_qual = bool(d.get("ia_score") or d.get("ia_opportunite"))
        has_valid_audit_or_ia = bool(has_valid_audit) or ia_qual
            
        # 5. Email prêt (audit valide OU qualification IA) et l'email complet
        has_email = (d.get("email_objet") and d.get("email_objet").strip()) and (d.get("email_corps") and d.get("email_corps").strip())
        if has_valid_audit_or_ia and has_email:
            return "email_genere"
            
        # 6. Audité (Succès uniquement)
        if has_valid_audit_or_ia:
            return "audite"
    ...
    def _get_statut_display(self, d: dict) -> dict:
        ...
        if s in ("en_attente", "audite"):
            ...
            # Afficher 'Audit Échoué' uniquement si un audit existe ET qu'il a réellement échoué
            # Ne pas afficher d'échec si l'IA a cependant fourni une qualification utile
            ia_qual_display = bool(d.get("ia_score") or d.get("ia_opportunite"))
            if d.get("audit_id") and d.get("audit_error"):
                return {"label": "Audit Échoué", "color": "#ef4444"}
            elif d.get("audit_id") and not score_valide and not template_special and not statut_force and not ia_qual_display:
                return {"label": "Audit Échoué", "color": "#ef4444"}
