    ...
    try:
        with get_conn() as conn:
            ...
            if existing:
                ...
                if lead.get('email_2') and not existing.get('email_2'):
                    updates['email_2'] = lead['email_2']
                ...
                if updates:
                    ...
                    conn.commit()
                # Si une campagne est fournie, assurer l'association avec une lead_list + lead_list_items
                try:
                    camp_id = lead.get('campaign_id')
                    if camp_id:
                        row = conn.execute("SELECT id FROM lead_lists WHERE campaign_id=? LIMIT 1", (camp_id,)).fetchone()
                        if row:
                            list_id = row['id']
                        else:
                            list_name = f"Auto-list campagne #{camp_id}"
                            cur = conn.execute(
                                "INSERT INTO lead_lists (nom, description, icone, campaign_id) VALUES (?, ?, ?, ?)",
                                (list_name, f"Auto-list pour campagne #{camp_id}", "🎯", camp_id)
                            )
                            list_id = cur.lastrowid
                            try:
                                conn.execute("UPDATE lead_lists SET objectif=? WHERE id=?", (lead.get('objectif', 'general'), list_id))
                            except Exception:
                                pass
                        try:
                            conn.execute("INSERT OR IGNORE INTO lead_list_items (list_id, lead_id) VALUES (?, ?)", (list_id, existing['id']))
                            conn.commit()
                        except Exception:
                            pass
                except Exception:
        ...
        cur = conn.execute("""
            ...
            (campaign_id, nom, adresse, site_web, telephone, email, email_2,
