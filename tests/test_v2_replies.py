...
def _prospect(oid, email='contact@dupont.fr'):
    ...
    r = prospects_repo.insert_prospect(
        get_or_create_liste(oid), nom='Boulangerie Dupont', email=email, ville='Cotonou',
...
def test_match_par_message_id(tmp_db):
    ...
    with get_conn() as conn:
        conn.execute(
            """INSERT INTO prospect_events (prospect_id, campagne_id, event_type, payload, message_id)
               VALUES (?,?, 'initial', '{}', ?)""",
...
def test_handle_ndr_pas_de_transition(tmp_db):
    ...
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO prospect_events (prospect_id, campagne_id, event_type, payload) VALUES (?,?, 'initial', '{}')",
            (pid, oid),
        )
        conn.commit()
    from core.orchestration import relances_due
    from envoi import template_registry
    template_registry.add_or_update(campagne_id=oid, position=1, delai_jours=0,
                                    objet='Re', corps='Re')
