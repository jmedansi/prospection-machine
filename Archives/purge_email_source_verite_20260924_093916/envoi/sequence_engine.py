...
def prepare_step(campagne_id, prospect_id, position, step_label, *, expected_statut=None,
    ...
    # contenu n'existe pour cette étape, l'envoi est BLOQUÉ (raison 'pas_email_ia').
    # `template_registry` ne sert plus qu'au rendu d'*inspiration* (prévisualisation
...
def _perform_send(campagne_id, prospect, objet, corps, *, template_id=None,
    ...
    # Preuve d'envoi : sans Message-ID RFC, on ne transitionne PAS (évite les
    # « prospects fantômes » en_sequence sans mail émis). Garde structurelle.
    if resp.get('message_id') is None and not dry_run:
        return {'success': False, 'statut': 'sans_message_id',
                'message': 'Envoi sans message_id — pas de transition', 'id': None}
...
def send_relance(campagne_id, prospect_id, *, force=False, humanize_on=True,
    ...
    # RÈGLE PRODUIT (structurelle) : TOUTE relance exige une validation Telegram.
    # Il n'y a aucun contournement par config (`validation_telegram` ne s'applique
    # qu'à l'INITIAL). Exceptions limitées :
    #   - `approval='auto'` : lot de relances déjà ✅ en amont (batch validator) ;
    #   - `dry_run`         : aucun envoi réel.
    needs_tg = approval != 'auto' and not dry_run
...
def approve_and_send_initial(prospect_id, dry_run=False) -> dict:
    ...
    # Contenu FRAIS = source de vérité (data_extra / colonnes email_* du prospect) :
    # on RELIT le contenu rédigé au moment de l'approbation au lieu du contenu
    # stocké dans la demande Telegram (qui peut dater et être obsolète). Si le
    # contenu frais a disparu (edition manuelle), le snapshot Telegram reste le
    # filet de sécurité — l'utilisateur a déjà validé ce texte.
    position = {'initial': 0, 'relance_1': 1, 'relance_2': 2, 'relance_3': 3}.get(step, 0)
    frais = get_custom_step_email(detail, position)
    if frais:
        objet, corps = frais
