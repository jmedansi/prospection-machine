...
w("")
w("### A. OCCURRENCES RESIDUELLES 'generate_email_for_lead' / 'email_generator' / 'build_premium_email' / 'email_builder' (hors archives)")

PATS = [
    ("generate_email_for_lead", None),
    ("services.email_generator", None),
    ("build_premium_email", None),
    ("email_builder", None),
...
# =============================================================
w("")
w("### F. VERIFICATION FINALE (imports services.email_generator / build_premium_email V1)")
leftover = []
for p in pys:
    with io.open(p, "r", encoding="utf-8", errors="ignore") as f:
        s = f.read()
    if "services.email_generator" in s or "from envoi.email_builder import build_premium_email" in s:
