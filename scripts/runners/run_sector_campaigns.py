...
def wait_for_campaigns_to_complete(campaign_ids: set, timeout_hours: int = 4, poll_seconds: int = 30):
    ...
    while time.time() < deadline:
        ...
        with get_conn() as conn:
            active = conn.execute(
                f"SELECT COUNT(*) FROM campagnes_legacy WHERE id IN ({ids_str}) AND phase IN ('pending', 'scraping', 'enrichment')"
            ).fetchone()[0]
        if active == 0:
...
def _campaign_exists(sector: str, keyword: str, city: str) -> bool:
    ...
    with get_conn() as conn:
        row = conn.execute(
            "SELECT id FROM campagnes_legacy WHERE nom=? AND phase NOT IN ('failed','stopped','cancelled')",
...
def launch_ads_for_sector(sector_cfg: Dict):
    ...
    for city in BIG_CITIES:
        ...
        if ok:
            ...
            with get_conn() as conn:
                row = conn.execute(
                    "SELECT id FROM campagnes_legacy WHERE nom=? ORDER BY id DESC LIMIT 1",
                    (f"Ads-{sector}-{city}",)
                ).fetchone()
                if row:
