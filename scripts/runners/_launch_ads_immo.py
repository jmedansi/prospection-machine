...
if ok:
    ...
    while True:
        c = get_conn().cursor()
        c.execute("SELECT id FROM campagnes_legacy WHERE nom='Ads-immobilier' ORDER BY id DESC LIMIT 1")
        row = c.fetchone()
        if row:
            camp_id = row[0]
            c.execute("SELECT phase, total_leads FROM campagnes_legacy WHERE id=?", (camp_id,))
            r = c.fetchone()
